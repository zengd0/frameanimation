...
public class ImageFrameHandler implements WorkHandler.WorkMessageProxy {
    ...
    ImageFrameHandler(@Operation int type, Context context, String assetFolderName) {
...
//    }
    ...
    private void setImageFrameAsset(final String[] files, int width, int height, int fps,
                                    OnImageLoadListener onPlayFinish) {
        this.width = width;
        this.height = height;
        if (imageCache == null) {
    ...
    private void setImageFrame(final String[] files, int width, int height, int fps,
                               OnImageLoadListener onPlayFinish) {
        this.width = width;
        this.height = height;
        if (imageCache == null) {
    ...
    private void load(final String[] files) {
        Message message = Message.obtain();
        message.obj = files;
        message.what = ASSET;
        workHandler.getHandler().sendMessage(message);
    ...
    public ImageFrameHandler setPlayCount(int playCount) {
    ...
    private void loadInThreadFromAsset(final String[] assetsFilePaths) {
        if (index < assetsFilePaths.length) {
            String file = assetsFilePaths[index];

            try {
//                mApplicationContext.getAssets().open(assetFolderName + "/" + file);//这行只是用来判断文件存不存在，没必要，因为下面decode那行也有同样效果
                if (bitmapDrawable != null) {
                    imageCache.mReusableBitmaps.add(new SoftReference<>(bitmapDrawable.getBitmap()));
                }
                long start = System.currentTimeMillis();
                bitmapDrawable =
                        BitmapLoadUtils.decodeSampledBitmapFromAsset(mApplicationContext, assetFolderName + "/" + file, width, height,
                                imageCache, isOpenCache);
                long end = System.currentTimeMillis();
                float updateTime = (frameTime - (end - start)) > 0 ? (frameTime - (end - start)) : 0;
                Message message = Message.obtain();
                message.what = ASSET;
                message.obj = assetsFilePaths;
                handler.sendMessageAtTime(message,
                        index == 0 ? 0 : (int) (SystemClock.uptimeMillis() + updateTime));
                index++;
            } catch (IOException e) {
                e.printStackTrace();
                index++;
                loadInThreadFromAsset(assetsFilePaths);
            }
    ...
    public void handleMessage(Message msg) {
        switch (msg.what) {
            ...
            case FILE:
                ...
                break;
            case ASSET:
                loadInThreadFromAsset((String[]) msg.obj);
                break;
        }
     ...
     * 一种是Resource
     */
    public static class AssetsHandlerBuilder implements FrameBuild {
        private int width;
        private int height;
        private int fps = 30;
        private String[] assetsFilePaths;
        private OnImageLoadListener onPlayFinish;
        private ImageFrameHandler imageFrameHandler;
        private int startIndex;
        private int endIndex;
        ...
        public AssetsHandlerBuilder(@NonNull String parentFolderName, Context context) {
            this.parentFolderName = parentFolderName;
            try {
                this.assetsFilePaths = context.getAssets().list(parentFolderName);
            } catch (IOException e) {
                e.printStackTrace();
        ...
        @Override
        public FrameBuild setStartIndex(int startIndex) {
            if (startIndex >= assetsFilePaths.length) {
                throw new IllegalArgumentException("startIndex is not  big to files length");
            }
            this.startIndex = startIndex;
            return this;
        ...
        @Override
        public FrameBuild setEndIndex(int endIndex) {
            if (endIndex > assetsFilePaths.length) {
                throw new IllegalArgumentException("endIndex is not  big to files length");
            }
            if (endIndex <= startIndex) {
                throw new IllegalArgumentException("endIndex is not to small startIndex");
        ...
        public ImageFrameHandler build() {
            if (!imageFrameHandler.isRunning) {
                clip();
                imageFrameHandler.setImageFrameAsset(assetsFilePaths, width, height, fps,
                        onPlayFinish);
            }
            return imageFrameHandler;
        }
        ...
        private void createHandlerAsset() {
            if (imageFrameHandler == null) {
                imageFrameHandler = new ImageFrameHandler(ASSET, context, parentFolderName);
            }
        }
        ...
        public FrameBuild clip() {
            if (startIndex >= 0 && endIndex > 0 && startIndex < endIndex) {
                assetsFilePaths = splitAsset(assetsFilePaths, startIndex, endIndex);
            }
            return this;
        }


        String[] splitAsset(String[] resArray, int start, int end) {
            String[] ints = new String[end - start];
            int index = 0;
            for (int i = start; i < end; i++) {
                ints[index] = resArray[i];
                index++;

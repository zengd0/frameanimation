...
public class BitmapLoadUtils {
   ...
   *
   */
  public static BitmapDrawable decodeSampledBitmapFromAsset(Context context,String filename,
                                                            int reqWidth, int reqHeight,
                                                            ImageCache cache, boolean isOpenLruCache) throws IOException {

    final BitmapFactory.Options options = new BitmapFactory.Options();
    options.inJustDecodeBounds = true;
    decodeStream(context.getAssets().open(filename), null,options);
    options.inSampleSize = calculateInSampleSize(options, reqWidth, reqHeight);
    ...
    if (isOpenLruCache) {
      ...
      if (bitmapFromCache == null) {
        // if (Utils.hasHoneycomb()) {
        bitmapFromCache = readBitmapFromAsset(context,filename, cache, options);
        cache.addBitmap(filename, bitmapFromCache);
      }
    } else {
      bitmapFromCache = readBitmapFromAsset(context,filename, cache, options);
  ...
  @NonNull
  private static BitmapDrawable readBitmapFromAsset(Context context,String filename, ImageCache cache,
    ...
    // If we're running on Honeycomb or newer, try to use inBitmap.
    options.inJustDecodeBounds = false;
    bitmapFromCache = new BitmapDrawable(BitmapFactory.decodeStream(context.getAssets().open(filename), null,options));
    return bitmapFromCache;
  }
